...
#    exit 1
  ...
  counter=0
  while [ -z "$CLUSTER_NAME" -a $counter -lt $ES_CONNECT_RETRY ]; do
    sleep 1
    ((counter++))
    CLUSTER_NAME=$(curl -k ${ELASTICSEARCH_URL}/_cat/health?h=cluster 2> /dev/null | tr -d '[:space:]')
    echo "Waiting for Elasticsearch cluster to respond ($counter/$ES_CONNECT_RETRY)"
  done
...
echo "Reset password for elastic"
printf "y\nelastic\n${ELASTIC_PASSWORD}\n" | /opt/elasticsearch/bin/elasticsearch-reset-password -u elastic -i

echo "Reset password for kibana_system"
...
# Copy the Elastic autogenerated certificate to the Logstash config folder
mkdir -p /etc/logstash/config/certs/
cp  /etc/elasticsearch/certs/http_ca.crt /etc/logstash/config/certs/
chown logstash:logstash /etc/logstash/config/certs/http_ca.crt

# WORKAROUND It seems like the user logstash doesn't acccess to the env var "${ELASTIC_PASSWORD}", which leads to this error:
# "Replacement variable `ELASTIC_PASSWORD` is not defined in a Logstash secret store or an environment entry and there is no default value given"
# NICE-TO-HAVE: set the password in a Logstash secret store.
sed -i -e "s/ELASTIC_PASSWORD/${ELASTIC_PASSWORD}/g" "/etc/logstash/conf.d/99 - Output.conf"

# Restart Logstash to apply the configuration changes
