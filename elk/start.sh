...
if [ "$ELASTICSEARCH_START" -ne "1" ]; then
  ...
  if [ -z "$ELASTICSEARCH_URL" ]; then
    ELASTICSEARCH_URL=${ES_PROTOCOL:-https}://localhost:9200
  fi
...
if [ "$LOGSTASH_START" -ne "1" ]; then
  ...
  # WORKAROUND It seems like the user logstash doesn't access to the env var "${ELASTIC_PASSWORD}", which leads to this error:
